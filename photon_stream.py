...
#REFERENCE: https://www.researchgate.net/publication/330429600_Behavioral_Modeling_of_Photon_Arrival_Time_for_Time-of-Flight_Measurement_Circuit_Simulation
...
def arrival_estimation(QE1, tof, SR, tau_laser_mean, laser_pulse_duration, tau_amb_mean, window_start, window_end, seed=None,
                       bin_width=0.1, fit_bins=7):
    if seed is not None:
        np.random.seed(seed)

    expected_photons = np.random.binomial(n=pulses, p=SR)
    raw_depth = tof + np.random.normal(0, jitter, size=expected_photons)

    detection_mask = np.random.rand(expected_photons) < QE1
    detected_photons = raw_depth[detection_mask]

    first_laser_arrival = np.min(detected_photons)
...
with open("tof_trials_results.csv", mode="w", newline="") as file:
    ...
    for trial in range(1, 31):
        ...
        n_s_ratio = np.random.uniform(60, 80) #60-80 : 1
        ...
        background_noise = int(n_s_ratio * len(signal_arrival))
